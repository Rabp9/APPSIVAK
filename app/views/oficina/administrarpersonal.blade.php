...
@section('contenidoCuerpo')
    ...
    <h2 class="textAlignRight bordeAbajo tituloCabecera">OFICINA: ADMINISTRAR PERSONAL</h2>
    <section class="contenidoTop contenedorGeneralDragAndDrop">
        <div class="formulario" style="user-select: none;">
            <section id="seccionOficina" class="contenidoTop" style="box-shadow: 0px 0px 4px rgba(0, 0, 0, 0.7)">
                <div class="tituloFormulario">
                    Oficina: {{{$tOficina->descripcion}}}
                </div>
            ...
            <form id="frmAsignarPersonalOficina" action="/APPSIVAK/public/oficina/administrarpersonal/{{{$tOficina->codigoOficina}}}" method="post">
                <input type="hidden" id="arrayAsignados" name="arrayAsignados">
                <input type="hidden" id="arrayEliminados" name="arrayEliminados">
                <div class="seccionBotones bordeArriba">
                    <input type="button" value="Guardar Cambios" onclick="enviarFrmAsignarPersonalOficina();">
                </div>
            </form>
        </div>
    </section>
    <script>
    function enviarFrmAsignarPersonalOficina()
    {
        if(confirm('Realmente desea guardar los cambios realizados'))
        {        
            $('#frmAsignarPersonalOficina').submit();
            return;
        }
